...
REM wait, the web server itself - Ctrl+C is how it is stopped - and the final pause) runs under the
REM caller's code page instead: answering Y to "Terminate batch job (Y/N)?" skips :finish and would
REM leave the caller's console in 65001. The app also gets the same console code page as with
REM new_run_main_ui.bat. Only ASCII lines may sit between a restore and the next switch to 65001.
set "URIV_PREV_CP="
...
REM 서버는 호출한 콘솔의 코드 페이지로 돈다(Ctrl+C → 일괄 작업 끝내기 Y 가 :finish 를 건너뛰어도 그대로다).
if defined URIV_PREV_CP "%SystemRoot%\System32\chcp.com" %URIV_PREV_CP% >nul
...
if not "%URIV_EXIT%"=="0" (
    ...
    echo ============================================================
    if exist logs\last_crash_web.log type logs\last_crash_web.log
    echo ============================================================
    set "URIV_PAUSE=1"
)
